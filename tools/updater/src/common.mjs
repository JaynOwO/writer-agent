...
import { existsSync, lstatSync, mkdirSync, openSync, closeSync, writeFileSync, fsyncSync, readFileSync, renameSync, unlinkSync, realpathSync, statSync } from 'node:fs';
...
/** Compare existing filesystem entries, not spellings of their paths.
 * The JS realpath implementation can retain a Windows 8.3 ancestor while Git
 * reports its long name. Native resolution plus bigint file identity also avoids
 * treating two case-sensitive Windows directories as equal by lowercasing them.
 * Missing/inaccessible entries still throw; this never relaxes ordinary()/tree checks.
 */
export function samePath(a, b) {
  const x = realpathSync.native(a), y = realpathSync.native(b);
  const left = statSync(x, { bigint: true }), right = statSync(y, { bigint: true });
  if (left.ino > 0n && right.ino > 0n) {
    return left.dev === right.dev && left.ino === right.ino &&
      (left.mode & 0o170000n) === (right.mode & 0o170000n);
  }
  // Filesystems without usable file IDs require an exact native spelling.
  // Do not fall back to a case-insensitive/prefix/basename comparison.
  return x === y;
}
