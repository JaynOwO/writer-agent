...
export const EXPECTED_DESKTOP_CHECKS = 18;
...
export async function finalizeSmoke(owner, child, { output, expectedElectron, remove, wait } = {}) {
  ...
  let report = null;
  // Read a run-bound partial report even on failure. Otherwise sixteen passed
  // scenarios followed by one capture error misleadingly became "checks: 0".
  try {
    ...
    if (candidate.runId !== owner.id || !Number.isSafeInteger(candidate.passed) || candidate.passed < 0 ||
        candidate.passed > EXPECTED_DESKTOP_CHECKS || !Array.isArray(candidate.checks) ||
        candidate.checks.length !== candidate.passed ||
        candidate.checks.some(x => typeof x !== 'string' || !x) || new Set(candidate.checks).size !== candidate.passed ||
        candidate.versions?.electron !== expectedElectron) throw Error('Missing, stale or incomplete desktop test results.');
    report = candidate;
    if (candidate.failure) {
      const { stage, message } = candidate.failure;
      errors.push(`Desktop scenario failed [${String(stage).slice(0, 200)}]: ${String(message).slice(0, 2000)}`);
    }
    if (candidate.passed !== EXPECTED_DESKTOP_CHECKS) errors.push('Incomplete desktop test results.');
