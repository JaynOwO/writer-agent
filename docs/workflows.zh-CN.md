...
本指南原来的“只取开头／没有工具”说明继续描述旧任务记录。schema6 向导新建任务默认带空 extensions 对象，启用按问题选段与引用映射；进入任务的工具／技能／章节菜单可加入指定 Skill、固定参数的 MCP 读取与可选逐章起草，刷新后需要新的阶段许可。MCP 与网页读取共用 fetches 额度。参见[英文扩展指南](extensions.md)／[简中扩展指南](extensions.zh-CN.md)，旧配置不会被悄悄升级。
