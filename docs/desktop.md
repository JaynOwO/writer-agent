...
## Smoke lifecycle correction (winfix4)

Run the fixture through `pnpm desktop:smoke`, not by opening `apps/desktop/test` directly. The external Node harness creates a unique marked test directory and run-ID-bound output directory under `.cache/desktop-test`. The Electron fixture still performs all 17 real-window scenarios, closes the writing service and reports scenario results, then exits. It never removes its own active Chromium userData/cache directory. Only after child `close` does the parent validate the exact run ID, Electron version and full results, and remove its own synthetic directory with finite transient-lock retries (5.5 seconds maximum waiting, excluding filesystem time). Persistent cleanup failures, changed ownership, incomplete results, timeout or nonzero exit remain failures. Unconfirmed process exit retains the directory for inspection. The final DESKTOP_E2E_OK marker is emitted by the parent after successful cleanup, not by the still-running Electron process.

On Linux GitHub Actions, the complete verified runtime is copied into a new root-owned CI directory and the exact official helper is installed adjacent to its Electron executable. SIGLUM_CI_RUNTIME selects that directory only for the explicit Linux CI job. Chromium's adjacent helper lookup takes priority over its development environment fallback; merely setting CHROME_DEVEL_SANDBOX while leaving an invalid helper adjacent is not sufficient. The application remains sandboxed, no system policy is weakened, and an always-run cleanup removes the CI-only privileged runtime. This is not a user installation action and never runs on Windows. The original runtime archive/cache bytes and production desktop packaging are unchanged.
