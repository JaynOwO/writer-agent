...
test('all six project manifests adopt Apache-2.0 and v0.0.5 without enabling npm publication',()=>{
  for(const dir of ['.','apps/cli','packages/core','packages/models','packages/storage','tests']){
    ...
    assert.equal(p.version,'0.0.5');assert.equal(p.license,'Apache-2.0');assert.equal(p.private,true);
  }
});
...
test('CI explicitly runs all five demos without configuring a real model key',()=>{
  const ci=readFileSync(join(root,'.github/workflows/ci.yml'),'utf8');assert.match(ci,/pnpm demo:provider/);assert.match(ci,/pnpm demo:sources/);assert.match(ci,/pnpm demo:review/);assert.match(ci,/pnpm demo:memory/);assert.match(ci,/pnpm install --frozen-lockfile/);assert.doesNotMatch(ci,/secrets\./);
});

test('public CLI and core demo identify Siglum v0.0.5 while repository names stay compatible',()=>{
  for(const file of ['apps/cli/src/index.ts','apps/cli/src/demo.ts'])assert.match(readFileSync(join(root,file),'utf8'),/Siglum v0\.0\.5/);
