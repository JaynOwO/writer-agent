...
for (const version of [1, 2] as const) {
    test(`schema v${version} preview and old workflows do not implicitly enable analysis`, t => { const { root, path } = legacy(t, version), before = rows(path); assert.equal(migrateWorkspace(root).to, 3); assert.deepEqual(rows(path), before); assert.equal(existsSync(join(root, '.writer/backups')), false); const w = Workspace.open(root); try {
    ...
    test(`schema v${version} -> v3 preserves all original rows and verifies a v${version} backup`, t => {
        ...
        try {
            assert.equal(w.info().schemaVersion, 3);
...
test('future schema v4 is rejected, not silently downgraded to v3', t => { const { root, path } = legacy(t, 2), db = new DatabaseSync(path); db.exec('PRAGMA user_version=4'); db.close(); expectCode(() => Workspace.open(root), 'UNSUPPORTED_SCHEMA'); expectCode(() => migrateWorkspace(root, true), 'UNSUPPORTED_SCHEMA'); });
