...
for(const version of [1,2,3] as const){
 test(`v${version} remains readable, previews v7 without writes and requires memory migration`,t=>{const {root,path}=legacy(t,version),before=rows(path);assert.equal(migrateWorkspace(root).to,7);assert.deepEqual(rows(path),before);assert.equal(existsSync(join(root,'.writer/backups')),false);const w=Workspace.open(root);try{expectCode(()=>w.memory.profiles(),'MIGRATION_REQUIRED');assert.equal(w.info().schemaVersion,version);}finally{w.close();}});
 test(`v${version} -> v7 backs up and preserves every old table row`,t=>{const {root,path,documentId,runId}=legacy(t,version),before=rows(path),m=migrateWorkspace(root,true);assert.ok(m.backup);assert.deepEqual(rows(m.backup),before);const after=rows(path);for(const [k,v]of Object.entries(before))assert.deepEqual(after[k],v,k);const w=Workspace.open(root);try{assert.equal(w.info().schemaVersion,7);const p=w.memory.createProfile('new');w.memory.addPreference(p.id,pref());w.memory.attach(documentId,p.id);w.memory.saveIntent(documentId,card());if(runId){const report=w.analysis.report(runId);assert.equal(report.request.memory,undefined);assert.equal(report.promptVersion,'analysis-v1');}assert.equal(w.history(documentId).length,1);}finally{w.close();}assert.equal(migrateWorkspace(root,true).needed,false);assert.equal(readdirSync(join(root,'.writer/backups')).length,1);});
}
