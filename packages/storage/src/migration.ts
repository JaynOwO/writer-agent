...
function versionOf(db:DatabaseSync):number {
  ...
  if(version!==1&&version!==2&&version!==3&&version!==4&&version!==5&&version!==6&&version!==SCHEMA_VERSION)throw new WriterError('UNSUPPORTED_SCHEMA','Unknown schema; no migration attempted.');return version;
}
...
export function migrateWorkspace(directory:string,apply=false) {
  ...
  try {
    ...
    if(version<6)db.exec(EXTENSION_SQL);
    if(version<7)db.exec(RANGE_SQL);
