...
import { McpClient, mcpLaunchHash, McpError, type McpCredential } from './mcp-client.js';
...
export const extensionsHelp = `Siglum extension/research preview — explicit trust, no automatic script installation
  ...
  siglum mcp renew <workspace> <serverId> [--apply]
...
export async function discoverMcp(w: Workspace, id: string, signal?: AbortSignal, credential?:McpCredential, assertCurrent?:()=>void): Promise<StoredCatalog> {
  ...
  const client = new McpClient(server.config, server.launchHash,credential);
  try {
    assertCurrent?.();const catalog = await client.discover(signal);assertCurrent?.();
...
export async function extensionsCommand(family: string, args: string[]) {
  ...
  try {
    if (family === 'skill') {
      ...
      else if(action==='renew'){count(a,2,3);const apply=flag(a[2],'--apply'),s=w.extensions.server(a[1]!),launchHash=await mcpLaunchHash(s.config);print(apply?w.extensions.registerServer(s.config,launchHash):{preview:true,config:s.config,launchHash,oldServer:s.id,newTrustRequired:true});}
