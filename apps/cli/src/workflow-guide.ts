...
import { DEFAULT_WORKFLOW_BUDGET, validateWorkflowConfig, renderWorkflowDraft, renderCitedDraft, WriterError } from '@writer-agent/core';
...
import { selectWorkflowExtensions, runExtensionsGuide } from './extensions-guide.js';
...
export async function workflowGuideCommand(args:string[]):Promise<void>{
  ...
  try{w=Workspace.open(args[0]!);if(w.info().schemaVersion<6){w.close();w=null;show(terminal.io,migrateWorkspace(args[0]!));if(!await yes(terminal.io,'Back up and explicitly migrate workspace to schema v6? / 备份并升级工作区？'))return;show(terminal.io,migrateWorkspace(args[0]!,true));w=Workspace.open(args[0]!);}await runWorkflowGuide(w,terminal.io,args[2]==='en'?'en':'zh-CN');}
