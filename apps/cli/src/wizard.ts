...
export async function runWizard(w:Workspace,io:WizardIO,lang:MenuLanguage='zh-CN'):Promise<void>{
  ...
  for(;;){
    ...
    const actions=['document','import','profile','intent','preferences','suggest','review','changes','distill','uses','reports','check','workflow'] as const;
    ...
    try{
      if(action==='workflow'){if(w.info().schemaVersion<5)throw new WriterError('MIGRATION_REQUIRED','Workflow requires schema v5; close this guide and use workflow guide for explicit backed-up migration.');const {runWorkflowGuide}=await import('./workflow-guide.js');await runWorkflowGuide(w,io,lang);}
