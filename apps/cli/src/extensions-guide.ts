  ...
  for(;;){
    ...
    try{
      if(action==='import-skill'){
        ...
        const [op]=await menuChoose(io,tr('操作','Operation'),['trust','revoke','discover','catalog','renew'] as const,x=>({trust:tr('明确信任这个程序／连接','Explicitly trust launch/connection'),revoke:tr('撤销信任','Revoke trust'),discover:tr('启动／连接并发现工具和资源','Start/connect and discover tools/resources'),catalog:tr('查看上次保存的列表，不连接','Inspect saved catalogue without connecting'),renew:tr('为当前程序／SDK重新注册连接（不自动信任）','Re-register current program/SDK version (not automatic trust)')}[x]));
        ...
        if(op==='renew'){const launchHash=await mcpLaunchHash(s.config);show(io,{config:s.config,launchHash,newTrustRequired:true});if(await yes(io,tr('建立一个新连接记录？旧历史不变，需单独信任','Create a new connection record? Old history remains; trust separately')))show(io,w.extensions.registerServer(s.config,launchHash));}
