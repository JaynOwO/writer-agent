...
import {readSmokeWorkspace} from '../../../scripts/desktop-smoke-support.mjs';
import {configureSmokeRendering,captureSmokeImage} from '../../../scripts/desktop-capture-support.mjs';
...
const testOwner=readSmokeWorkspace(),temp=testOwner.root;
app.setPath('userData',join(temp,'ui'));
const rendering=configureSmokeRendering(app,{platform:process.platform,headless:process.env.SIGLUM_E2E_XVFB==='1'});
registerScheme();let host,passed=0;
const checks=[];let currentStage='startup',capture=null;let fixtureServer;const modelRequests=[];const credential='SYNTHETIC_DESKTOP_FIXTURE_NOT_A_REAL_KEY';
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
async function check(name,fn){currentStage=name;await fn();passed++;checks.push(name);console.log('PASS',name);}
...
try{
 ...
 await check('real compositor screenshot is nonempty and saved as PNG',async()=>{
  const {png,...metadata}=await captureSmokeImage(host.window);capture={...metadata,rendering};
  writeFileSync(join(process.env.SIGLUM_TEST_OUTPUT||temp,'desktop-e2e.png'),png);
  console.log('DESKTOP_CAPTURE_OK',JSON.stringify(capture));
 });
 ...
 writeFileSync(join(process.env.SIGLUM_TEST_OUTPUT||temp,'desktop-e2e.json'),JSON.stringify({runId:testOwner.id,passed,checks,versions:process.versions,capture,rendering,fixture:'owned synthetic data; composition events, not actual OS IME; no remote services'},null,2));
 fixtureServer?.closeAllConnections();await new Promise(r=>fixtureServer?fixtureServer.close(r):r());console.log('DESKTOP_SCENARIOS_OK',passed);app.exit(0); // Parent cleans userData only after process close.
}catch(e){
 const failure={stage:currentStage,message:String(e?.message||e),code:e?.code||null};
 try{writeFileSync(join(process.env.SIGLUM_TEST_OUTPUT||temp,'desktop-e2e.json'),JSON.stringify({runId:testOwner.id,passed,checks,versions:process.versions,capture,rendering,failure},null,2));}catch(reportError){console.error('DESKTOP_REPORT_FAILED',reportError.message);}
 console.error('DESKTOP_E2E_FAILED',JSON.stringify(failure),e.stack);try{await host?.shutdown();}catch{}fixtureServer?.closeAllConnections();fixtureServer?.close();app.exit(1);}
}
run();
